...
impl ScriptEngine {
    ...
    pub fn new(source: &str) -> Result<Self> {
        ...
        let ast = match engine.compile(source) {
            Ok(ast) => ast,
            Err(e) => return Err(anyhow!("Script compilation failed: {}", e.to_string())),
        };
        ...
        if !source.trim().is_empty() {
            if let Err(e) = engine.run_ast_with_scope(&mut scope, &ast) {
               return Err(anyhow!("Script execution failed: {}", e.to_string()));
            }
        }
    ...
    pub fn call_str(&mut self, fn_name: &str, arg: &str) -> Result<String> {
        ...
        match self.engine.call_fn::<Dynamic>(&mut self.scope, &self.ast, fn_name, (arg.to_string(),)) {
            Ok(result) => Ok(result.to_string()),
            Err(e) => Err(anyhow!("Function '{}' call failed: {}", fn_name, e.to_string())),
        }
    }
    ...
    pub fn call_function(&mut self, fn_name: &str, args: Vec<Dynamic>) -> Result<String> {
        let result = match args.len() {
            0 => {
                match self.engine.call_fn::<Dynamic>(&mut self.scope, &self.ast, fn_name, ()) {
                    Ok(r) => r,
                    Err(e) => return Err(anyhow!("Call failed: {}", e.to_string())),
                }
            }
            1 => {
                match self.engine.call_fn::<Dynamic>(&mut self.scope, &self.ast, fn_name, (args[0].clone(),)) {
                    Ok(r) => r,
                    Err(e) => return Err(anyhow!("Call failed: {}", e.to_string())),
                }
            }
            2 => {
                match self.engine.call_fn::<Dynamic>(&mut self.scope, &self.ast, fn_name, (args[0].clone(), args[1].clone())) {
                    Ok(r) => r,
                    Err(e) => return Err(anyhow!("Call failed: {}", e.to_string())),
                }
            }
    ...
    pub fn eval(&mut self, expr: &str) -> Result<String> {
        match self.engine.eval_with_scope::<Dynamic>(&mut self.scope, expr) {
            Ok(result) => Ok(result.to_string()),
            Err(e) => Err(anyhow!("Evaluation failed: {}", e.to_string())),
        }
    }
}
