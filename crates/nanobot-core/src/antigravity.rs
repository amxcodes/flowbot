...
impl CompletionModel for AntigravityCompletionModel {
        ...
        if use_openai_format {
            ...
            if messages.is_empty() {
                return Err(CompletionError::RequestError(
                    "Antigravity request contained no messages"
                        .to_string()
                        .into(),
                ));
            }
    ...
    ) -> Result<rig::streaming::StreamingCompletionResponse<Self::StreamingResponse>, CompletionError>
    {
        ...
        eprintln!(
            "DEBUG: Enabling {} tools for streaming request",
            tool_declarations.len()
        );
...
async fn stream_with_fallback(
    ...
    std::pin::Pin<
        Box<
            dyn futures::Stream<
                    Item = Result<
                        RawStreamingChoice<AntigravityStreamingResponse>,
                        CompletionError,
                    >,
                > + Send,
        >,
...
> {
    ...
    if is_cloudcode && let Some(token) = &auth_token {
        ...
        if let Some(pid) = load_project_id(client, &prod_base_url, token).await {
            ...
            eprintln!(
                "DEBUG: Discovery failed, using fallback: {}",
                resolved_project_id
            );
        }
        ...
        if let Some(models) =
            fetch_available_models(client, &prod_base_url, token, &resolved_project_id).await
        {
            available_models = models;
            eprintln!(
                "DEBUG: Activation sequence complete. Models found: {}",
    ...
    for retry_count in 0..max_retries {
        ...
        let url = if is_cloudcode {
            ...
            let gen_url = base_urls
                .iter()
                .find(|u| u.contains("generativelanguage"))
                .unwrap_or(&base_urls[0]);
        ...
        eprintln!(
            "DEBUG: Trying Project: '{}' (Sequence 1/1)",
            resolved_project_id
        );
        ...
        eprintln!(
            "DEBUG: Using DeepMind Signature Mode for Chat: {}",
            resolved_project_id
        );
        ...
        let random_suffix = format!(
            "{:09x}",
            (nanos as u64) ^ (retry_count * 99999) ^ ((timestamp as u64) & 0xFFFFFF)
        );
        let suffix_9 = if random_suffix.len() > 9 {
            &random_suffix[random_suffix.len() - 9..]
        } else {
            &random_suffix
        };
        ...
        eprintln!(
            "DEBUG: Extracted User Message for Role Alternation: {}",
            user_msg
        );
...
User: {}"#,
        ...
        match request_builder.send().await {
            Ok(resp) => {
                if resp.status().is_success() {
                    ...
                    let stream = resp.bytes_stream().eventsource().map(|event_res| {
                        match event_res {
                            Ok(event) => {
                                let data = event.data;
                                ...
                                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&data) {
                                    // Standard Response
                                    if let Some(text) = json["candidates"][0]["content"]["parts"][0]
                                        ["text"]
                                        .as_str()
                                    {
                                        ...
                                        json["candidates"][0]["content"]["parts"].as_array()
                                    {
                                        for part in parts {
                                            if let Some(function_call) = part.get("functionCall") {
                                                ...
                                                {
                                                    return Ok(RawStreamingChoice::Message(
                                                        format!("__TOOL_CALL__{}", json_str),
                                                    ));
                                                }
                                            }
                                        }
                                    ...
                                    if let Some(resp_obj) = json.get("response") {
                                        if let Some(text) =
                                            resp_obj["candidates"][0]["content"]["parts"][0]["text"]
                                                .as_str()
                                        {
                                            return Ok(RawStreamingChoice::Message(
                                                text.to_string(),
                                        ...
                                        if let Some(parts) =
                                            resp_obj["candidates"][0]["content"]["parts"].as_array()
                                        {
                                            for part in parts {
    ...
    Err(last_error
        .unwrap_or_else(|| CompletionError::ProviderError("Stream connection failed".into())))
}
...
async fn load_project_id(client: &reqwest::Client, base_url: &str, token: &str) -> Option<String> {
    ...
    let response = client
        ...
        .header(
            "Client-Metadata",
            serde_json::to_string(&request_body.metadata).unwrap_or_default(),
        )
        .json(&request_body)
        .send()
        .await;
    ...
    match response {
        Ok(resp) => {
            if !resp.status().is_success() {
                eprintln!(
                    "DEBUG: load_project_id failed with status: {}",
                    resp.status()
                );
...
async fn fetch_available_models(
    ...
    eprintln!(
        "DEBUG: Activation step (fetchAvailableModels) for project: {}",
        project_id
    );
        ...
        for url in build_candidate_urls(base_url) {
            ...
            if url.contains("cloudcode-pa") {
                if let Some(token) = &auth_token {
                    ...
                    let project_to_use =
                        discovered_id.unwrap_or_else(|| "rising-fact-p41fc".to_string());
                    ...
                    if let Some(models) =
                        fetch_available_models(client, base_url, token, &project_to_use).await
                    {
            ...
            for (i, project_id) in project_ids.iter().enumerate() {
                ...
                loop {
                    let pid_str = if project_id.is_empty() {
                        None
                    } else {
                    ...
                    let random_suffix = format!(
                        "{:09x}",
                        (nanos as u64)
                            ^ (retry_count as u64 * 99999)
                            ^ ((timestamp as u64) & 0xFFFFFF)
                    );
...
User: {}"#,
                        ...
                        if !parts.is_empty() {
                            ...
                            // Merge identity into the first part
                            let current_text =
                                first_part.text.as_ref().cloned().unwrap_or_default();
                            first_part.text =
                                Some(format!("{}\n\n{}", identity_block, current_text));
                            // Remove other parts to enforce Single-Part rule
                            parts.truncate(1);
                    ...
                    */
                    ...
                    request_builder =
                        request_builder.header("User-Agent", "antigravity/1.99.0 linux/x64");
                    ...
                    request_builder =
                        request_builder.header("Client-Metadata", client_metadata.to_string());
                        ...
                        Err(e) => {
                            if retry_count >= max_retries {
                                if i == project_ids.len() - 1 {
                                    last_error =
                                        Some(CompletionError::ProviderError(e.to_string()));
                                }
                                break; // Move to next project/url
                            }
                            retry_count += 1;
                            eprintln!(
                                "DEBUG: Got 429/Error. Retrying ({}/{}) on same project...",
                    ...
                    if !response.status().is_success() {
                            ...
                            && retry_count < max_retries
                        {
                            ...
                            eprintln!(
                                "DEBUG: Got {} from project: {}. Body: {}",
                                status, project_id, text
                            );
                            tokio::time::sleep(tokio::time::Duration::from_millis(backoff_ms))
                                .await;
                            backoff_ms *= 2;
                            ...
                            match serde_json::from_str::<serde_json::Value>(json_str) {
                                Ok(chunk) => {
                                    ...
                                    if let Some(response_obj) = chunk.get("response") {
                                        ...
                                        if let Some(candidates) = response_obj
                                            .get("candidates")
                                        ...
                                        match serde_json::from_value::<GeminiResponse>(
                                            response_obj.clone(),
                                        ) {
                                            Ok(parsed_response) => {
                                            ...
                                            Err(e) => {
                                                eprintln!(
                                                    "DEBUG: Failed to parse SSE chunk as GeminiResponse: {}.",
                                                    e
                                                );
                                            }
                                        }
                                ...
                                Err(e) => {
                                    eprintln!(
                                        "DEBUG: Failed to parse SSE JSON: {}. Line: {}",
                                        e, json_str
                                    );
                                }
                            }
                    ...
                    eprintln!(
                        "DEBUG: Total text parts collected: {}",
                        all_text_parts.len()
                    );
                    ...
                    if let Some(mut final_response) = aggregated_response {
                        ...
                        return Err(CompletionError::ResponseError(
                            "No valid response chunks found in SSE stream".to_string(),
                        ));
                    }
                } // End Retry Loop
            ...
            for (i, project_id) in project_ids.iter().enumerate() {
                ...
                loop {
                    ...
                    if !response.status().is_success() {
                        ...
                        {
                            eprintln!(
                                "DEBUG: Got {}, retrying ({}/{}) for project: {}",
                                status,
                                retry_count + 1,
                                max_retries,
                                project_id
...
fn build_candidate_urls(base_url: &str) -> Vec<String> {
    ...
    if base.contains("cloudcode-pa.googleapis.com")
        || base.contains("cloudcode-pa.sandbox.googleapis.com")
    {
...
fn message_to_chat_message(message: Message) -> Option<ChatMessage> {
    match message {
        ...
        Message::Assistant { content, .. } => {
            assistant_content_to_text(&content).map(|text| ChatMessage {
