...
use crate::skills::{SkillLoader, config::{SkillsConfig, SkillSetupWizard}};
...
impl SkillsTUI {
    ...
    fn load_skills(&mut self) -> Result<()> {
        ...
        self.skills = loader.skills().values().map(|skill| {
            ...
            terminal.draw(|f| {
                ...
                let skills: Vec<ListItem> = self
                    ...
                    .map(|(i, skill)| {
                        ...
                        let status_color = if skill.enabled { Color::Green } else { Color::Red };
                ...
                let skills_list = List::new(skills)
                    .block(Block::default().borders(Borders::ALL).title("Skills (↑↓:Navigate  e:Enable  d:Disable  s:Setup  r:Refresh  q:Quit)"));
