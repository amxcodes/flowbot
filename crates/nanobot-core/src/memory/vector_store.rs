...
impl MemoryManager {
    ...
    pub async fn add_document(
        ...
        self.add_document_with_vector(content, metadata, &vector)
            .await
    }
    ...
    pub async fn add_documents_batch(
        ...
        for chunk in documents.chunks(batch_size) {
            ...
            for (i, (content, metadata)) in chunk.iter().enumerate() {
                self.add_document_with_vector(content, metadata.clone(), &embeddings[i])
                    .await?;
                added += 1;
            }
    ...
    pub async fn search(&self, query: &str, limit: usize) -> Result<Vec<(f32, VectorEntry)>> {
        ...
        let vector_paths: Vec<String> = vector_results
            .iter()
            .map(|(_, e)| e.metadata.get("path").cloned().unwrap_or_default())
            .collect();
            ...
            for path in keyword_results {
                if !vector_paths.contains(&path) {
                    ...
                    if let Some(entry) = store
                        .iter()
                        .find(|e| e.metadata.get("path").map(|p| p.as_str()) == Some(&path))
                    {
                        let score = cosine_similarity(query_embedding, &entry.vector);
