use rig::completion::{CompletionModel, CompletionRequest, CompletionResponse, AssistantContent, Usage, CompletionError};
...
impl MetaClient {
    pub async fn new(config: LLMConfig) -> anyhow::Result<Self> {
        let antigravity_client = if config.antigravity.as_ref().map(|c| c.enabled).unwrap_or(false) {
...
impl CompletionModel for MetaCompletionModel {
                ...
                "openai" => {
                    if let Some(openai_cfg) = &client.config.openai {
                        if openai_cfg.enabled {
                            ...
                            if let Ok(oa_client) = rig::providers::openai::Client::new(&openai_cfg.api_key) {
    ...
    async fn completion(
        ...
        let start_idx = self.current_index.load(std::sync::atomic::Ordering::Relaxed);
        ...
        for i in 0..chain_len {
            ...
            let result = match provider {
                MetaInnerModel::Antigravity(m) => {
                     m.completion(req_clone).await
                      .map(|resp| (extract_text_from_choice(&resp.choice), resp.usage))
                      .map_err(|e| CompletionError::ProviderError(e.to_string()))
                },
                MetaInnerModel::OpenAI(m) => {
                     m.completion(req_clone).await
                      .map(|resp| (extract_text_from_choice(&resp.choice), resp.usage))
                      .map_err(|e| CompletionError::ProviderError(e.to_string()))
                }
            ...
            match result {
                Ok((content, usage)) => {
                    if idx != start_idx {
                        self.current_index.store(idx, std::sync::atomic::Ordering::Relaxed);
    ...
    async fn stream(
        ...
        _request: CompletionRequest,
         ...
         Err(CompletionError::ProviderError(
            "Streaming not yet implemented for MetaProvider".to_string()
        ))
    }
}
