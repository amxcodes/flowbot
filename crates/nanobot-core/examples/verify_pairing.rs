...
async fn main() -> anyhow::Result<()> {
    ...
    // Initialize DB (uses ~/.nanobot/pairing.db by default)
    ...
    if !is_auth {
        ...
        let code =
            pairing::create_pairing_request(channel, user_id.to_string(), username.clone()).await?;
        ...
        assert_eq!(
            pending_code,
            Some(code.clone()),
            "Pending code should match generated code"
        );
