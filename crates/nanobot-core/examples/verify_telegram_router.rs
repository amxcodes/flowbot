...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Router sent session '{}' to agent '{}'",
        telegram_session, agent_id
    );

    // 4. Verify
    assert_eq!(
        agent_id, "agent-1",
        "Router should assign an agent to the Telegram session"
    );

    println!("✅ Router correctly handles Telegram session keys (Generic Logic)");
    println!(
        "⚠️ NOTE: This test confirms the Router *logic* works, but the 'TelegramBot' struct itself isn't stored in the Router for OUTBOUND dispatch."
    );
