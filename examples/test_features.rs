...
async fn main() -> anyhow::Result<()> {
    ...
    match execute_tool(&fetch_call, None, None, None).await {
        Ok(content) => {
    ...
    let spawn_result = execute_tool(&spawn_call, None, None, None).await?;
    ...
    let output1 = execute_tool(&read_call, None, None, None).await?;
    println!("--- Output 1 ---\n{}", output1);

    // Wait more
    sleep(Duration::from_secs(2)).await;
    let output2 = execute_tool(&read_call, None, None, None).await?;
    println!("--- Output 2 ---\n{}", output2);

    // List processes
    let list_call = json!({ "tool": "list_processes" }).to_string();
    let list_out = execute_tool(&list_call, None, None, None).await?;
    ...
    match execute_tool(&input_call, None, None, None).await {
    ...
    execute_tool(&kill_call, None, None, None).await?;
    println!("✅ Process killed.");
    
    // Verify list empty (or exited)
    let list_out_final = execute_tool(&list_call, None, None, None).await?;
