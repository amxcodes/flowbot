...
pub static PROCESS_MANAGER: Lazy<Arc<ProcessManager>> = Lazy::new(|| {
...
impl ProcessManager {
    ...
    fn append_log(buffer: &Arc<Mutex<Vec<String>>>, line: String) {
        ...
        if lock.len() >= 100 { // Max 100 lines history
            lock.remove(0);
        }
        lock.push(line);
    }
    ...
    pub async fn write_input(&self, pid: u32, input: String) -> Result<()> {
        ...
        tx.send(input).await.map_err(|_| anyhow!("Failed to send input"))
    }
...
pub async fn spawn_process(args: SpawnArgs) -> Result<String> {
    ...
    let pid = PROCESS_MANAGER.spawn(args.command.clone(), args_vec).await?;
