    ...
    match tool_name {
        ...
        "read_process_output" => {
            ...
            let pid = if let Some(n) = pid_val.as_u64() {
                ...
                s.parse::<u32>().map_err(|_| anyhow::anyhow!("Invalid PID format"))?
            } else {
        ...
        "memory_save" => {
            ...
            match memory_manager {
                Some(manager) => {
                    manager.add_document(content, std::collections::HashMap::new()).await?;
        ...
        "cron" => {
            match cron_scheduler {
                Some(scheduler) => {
                    crate::tools::cron::execute_cron_tool(scheduler, &tool_call).await
                }
                None => {
...
pub fn is_tool_call(response: &str) -> bool {
    ...
    trimmed.starts_with("__TOOL_CALL__") || (trimmed.starts_with('{') && trimmed.contains(r#""tool""#))
}
...
mod tests {
    ...
    async fn test_tool_parsing() {
        let json = r#"{"tool": "run_command", "command": "echo", "args": ["hello"], "use_docker": false}"#;
