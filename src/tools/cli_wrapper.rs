use anyhow::Result;
use super::commands::{run_command, RunCommandArgs, CommandOutput};
...
pub async fn run_and_format(mut args: RunCommandArgs) -> Result<String> {
    ...
    Ok(format_output(output, &args.command, &CliOutputConfig::default()))
}
...
pub fn format_output(output: CommandOutput, command_name: &str, config: &CliOutputConfig) -> String {
