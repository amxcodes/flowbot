...
impl OAuthFlow {
    ...
    pub fn parse_redirect_url(&self, redirect_url: &str) -> Result<(String, String)> {
        ...
        let code = code.ok_or_else(|| anyhow::anyhow!("No 'code' parameter found in redirect URL"))?;
            ...
            "antigravity" => {
                ...
                Ok(ProviderToken {
                    access_token: token_data["access_token"]
                        .as_str()
                        .ok_or_else(|| anyhow::anyhow!("No access_token in response. Raw response: {:?}", token_data))?
                        .to_string(),
    ...
    pub async fn complete_flow(&mut self, redirect_url: &str) -> Result<()> {
        ...
        if let Some(verifier) = &self.code_verifier
            && state != *verifier {
               return Err(anyhow::anyhow!("State mismatch! Possible CSRF attack. Expected: {}, Got: {}", verifier, state)); 
            }
