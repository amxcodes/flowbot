...
use rig::completion::{CompletionRequest, Message, CompletionModel};
use rig::client::CompletionClient;
use rig::completion::message::{UserContent, Text, AssistantContent};
...
impl AgentProvider {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<StreamedAssistantContent<String>, rig::completion::CompletionError>> + Send>>, rig::completion::CompletionError> {
        match self {
                ...
                let mapped = stream.map(|res| {
                    res.map(|content| {
                        match content {
                            ...
                            StreamedAssistantContent::ToolCall(t) => StreamedAssistantContent::ToolCall(t),
                 ...
                 let mapped = stream.map(|res| {
                    res.map(|content| {
                        match content {
                            StreamedAssistantContent::Text(t) => StreamedAssistantContent::Text(t),
                            StreamedAssistantContent::ToolCall(t) => StreamedAssistantContent::ToolCall(t),
                            StreamedAssistantContent::Final(_) => StreamedAssistantContent::Final(String::new()), // Discard Final object
                            _ => StreamedAssistantContent::text(""), 
                        }
                    })
                });
...
pub struct AgentLoop {
    ...
    last_interaction: std::sync::Arc<tokio::sync::Mutex<Option<(String, mpsc::Sender<StreamChunk>)>>>,
}
    ...
    pub async fn new() -> Result<Self> {
        ...
        let provider = match default {
            "antigravity" => {
             ...
             "openrouter" => {
                let api_key = config.providers.openrouter.as_ref()
                ...
                AgentProvider::OpenAI(client.completions_api().completion_model("google/gemini-2.0-flash-001"))
            },
            "openai" | _ => {
                 let api_key = std::env::var("OPENAI_API_KEY")
                    .or_else(|_| config.providers.openai.as_ref().map(|c| c.api_key.clone()).ok_or(String::new())) 
        ...
        let cron_scheduler = crate::cron::CronScheduler::new(db_path.clone(), cron_event_tx).await?;
        cron_scheduler.start().await?;
        
        // Initialize Agent Manager
        let agent_manager = std::sync::Arc::new(
            crate::gateway::agent_manager::AgentManager::new()
        );
        ...
        let memory_db_path = PathBuf::from(&home).join(".nanobot").join("memory").join("index.json");
        ...
        let memory_manager = std::sync::Arc::new(
            crate::memory::MemoryManager::new(memory_db_path, mem_provider)
        );
        ...
        let workspace_watcher = match crate::memory::WorkspaceWatcher::new(watch_path.clone(), memory_manager.clone()) {
            Ok(w) => {
        ...
        let personality = if workspace_dir.exists() {
            match personality::PersonalityContext::load(&workspace_dir).await {
                Ok(p) => {
                    eprintln!("✨ Loaded personality: {} {}", p.agent_emoji(), p.agent_name());
                    Some(p)
                }
                Err(e) => {
                    ...
                    match event {
                        crate::cron::CronEvent::SystemEvent { job_id, text } => {
                            ...
                            if let Some((session_id, response_tx)) = interaction {
                                ...
                                println!("⚠️ No active interaction found for SystemEvent injection.");
                            }
                        }
                        crate::cron::CronEvent::AgentTurn { job_id, message, model, .. } => {
                            println!("📅 [Cron] AgentTurn from job {}: {} (model: {:?})", 
                            ...
                            tokio::spawn(async move {
                                println!("🔄 [Cron] Executing isolated agent for job {}...", job_id_clone);
                                ...
                                match crate::cron::isolated_agent::run_isolated_agent_turn(
                                    ...
                                    Ok(result) => {
                                        println!("✅ [Cron] {} Isolated agent completed: {:?}", 
                                        ...
                                        if let Some(output) = result.output_text {
                                            ...
                                            if let Some((session_id, response_tx)) = interaction {
                                                println!("💉 [Cron] Injecting result into session {}", session_id);
                                                let _ = response_tx.send(crate::agent::StreamChunk::TextDelta(
                                                    format!("\n\n{}\n\n", feedback_msg)
                                                )).await;
                                            } else {
                                                println!("⚠️ [Cron] No active session for feedback injection");
                                            }
                                        }
                                    ...
                                    Err(e) => {
                                        eprintln!("❌ [Cron] Failed to execute isolated agent: {}", e);
                                    }
                                }
                            });
                        }
    ...
    async fn process_streaming(&self, msg: AgentMessage) {
        ...
        loop {
            ...
            let mut stream = match self.provider.stream(request).await {
                Ok(s) => s,
                Err(e) => {
                    let _ = msg.response_tx.send(StreamChunk::TextDelta(format!("Error: {}", e))).await;
                    break;
                }
            };
                    ...
                    Ok(chunk) => {
                         match chunk {
                             StreamedAssistantContent::Text(text) => {
                                 ...
                                 let _ = msg.response_tx.send(StreamChunk::TextDelta(text.text)).await;
                             }
            ...
            if !current_text.is_empty() {
                 ...
                 chat_history.push(Message::Assistant { 
                     id: None,
                     content: OneOrMany::one(AssistantContent::Text(Text{text: current_text.clone()}))
                 });
            }
            ...
            for tool_call in tool_calls {
                let _ = msg.response_tx.send(StreamChunk::ToolCall(tool_call.function.name.clone())).await;
                ...
                let result_str = match tool_call.function.name.as_str() {
                    "cron" => {
                        match crate::tools::cron::execute_cron_tool(&self.cron_scheduler, &args_value).await {
                             Ok(res) => res,
                ...
                let _ = msg.response_tx.send(StreamChunk::ToolResult(result_str.clone())).await;
