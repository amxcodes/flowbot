...
impl TokenManager {
    ...
    pub async fn get_token(&self) -> Result<String> {
        ...
        let Some(refresh_token) = refresh_token_str else {
            return Err(anyhow!("No refresh token available for {}. PLease login again.", self.provider));
        };

        // Perform Refresh
        eprintln!("DEBUG: Refreshing expired/missing token for provider: {}", self.provider);
