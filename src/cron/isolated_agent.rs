...
pub async fn run_isolated_agent_turn(
    ...
    match tokio::time::timeout(timeout, execute_agent_message(&session.id, &message, model_override)).await {
        Ok(Ok(output)) => {
...
async fn execute_agent_message(
    ...
    let client = crate::antigravity::AntigravityClient::from_env().await
    ...
    let chat_history = vec![Message::User {
        content: OneOrMany::one(UserContent::Text(Text { text: message.to_string() }))
    }];
